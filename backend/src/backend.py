...
import grpc
import cv2
from Inference import ModelInference as predictModule
...
class BackendService(backend_pb2_grpc.BackendServicer):
    ...
    def predicting(self, request, context):
        path = request.path
        print('hola')
        image_str, w, h = self._test_func(path=path) 
       
        prediction = predictModule.predict_label(str(path))
        
        response_message = backend_pb2.inferencia(val_inferencia=prediction)
        return response_message
